...
深色玻璃态设计风格，导航栏提供「首页 / 证明助手 / 学习资料」三个入口。Hero 区展示系统定位，功能卡片一览三大核心模块。

---

### 证明助手

![证明助手](docs/images/screenshot-prover.png)

页面分为左右两栏：

| 区域 | 说明 |
|------|------|
| **左栏 — 输入面板** | ① **前提（Premises）**：可添加多条前提公式；② **证明目标（Goal）**：填入待证结论；③ **证明风格**：可选自然演绎 / 真值表 / 语义树；④ **符号键盘**：点击即可向当前光标位置插入逻辑符号；⑤ **示例按钮**：一键载入经典例题（MP、HS、CP、RAA 等） |
| **右栏 — 证明结果** | 顶部状态横幅显示成功/失败；中部以 **Fitch 风格 logicproof 表格**逐步列出每条推理，嵌套子证明由紫色竖线标示深度；每行右侧「解释」按钮可调用 AI 解释该步骤；底部提供可直接复制的 LaTeX 代码 |

使用流程：在左栏输入前提 → 填写目标公式 → 选择证明风格 → 点击「开始证明」→ 在右栏查看逐步证明结果。

---

### AI 解释功能

![AI 解释](docs/images/screenshot-ai-explain.png)

点击任意证明步骤右侧的「解释」按钮，系统调用 Claude API 在该步骤下方展开 AI 解释行（紫色左边框高亮）。解释内容包含：所用推理规则的定义、该步骤引用的具体行号与公式、以及推理的逻辑依据，全程中文输出。

---

### 学习资料

![学习资料](docs/images/screenshot-learn.png)

左侧固定目录导航，点击可快速跳转各章节：命题逻辑基础、∧ 合取规则、双重否定、→-elim 蕴含消除、→-intro 条件证明、∨ 析取规则、¬ 否定规则。正文内嵌渲染的真值表（带框线 HTML 表格）和 KaTeX 数学公式，系统化呈现每条推理规则的形式定义与使用示例。
